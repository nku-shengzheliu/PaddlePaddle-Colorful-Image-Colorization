...
class NNEncLayer(object):
    ...
    def forward(self, x):
        ...
        max_encode = np.argmax(encode, axis=1).astype(np.int64)  # [bs, 56, 56]，用于Eq.(2)的loss计算
...
def flatten_nd_array(pts_nd, axis=1):
    ...
    bs, c, h, w = pts_nd.shape 
    pts_flt = pts_nd.transpose((0,2,3,1))
    pts_flt = pts_flt.reshape([bs*h*w, c])
    # NDIM = pts_nd.dim()  # 4
    # SHP = np.array(pts_nd.shape)  # array([bs, 2, 56, 56])
    ...
    # pts_flt = pts_nd.transpose(axorder)  # pytorch, [bs, 56, 56, 2]
    # pts_flt = pts_flt.reshape([NPTS.item(), SHP[axis].item()])  # pytorch, [bs*56*56, 2]
    return pts_flt
...
def decode(data_l, conv8_313, rebalance=1):
    ...
    '''
    data_l = data_l[0]+50  # 恢复到原来的明度范围
    data_l = data_l.cpu().numpy().transpose((1, 2, 0))  # [256, 256, 1] 
    conv8_313 = conv8_313[0]  # [313, 64, 64]
    conv8_313_rh = conv8_313 * rebalance
    class8_313_rh = F.softmax(conv8_313_rh, axis=0).cpu().numpy().transpose((1, 2, 0))  # [64, 64, 313]
    
    class8 = np.argmax(class8_313_rh, axis=-1)  # 找到最大的量化值索引
    
    enc_dir = 'models/resources'
    cc = np.load(os.path.join(enc_dir, 'pts_in_hull.npy'))
    #data_ab = np.dot(class8_313_rh, cc)
    data_ab = cc[class8]  # [64, 64, 2]
    ...
    img_lab = np.concatenate((data_l, data_ab), axis=-1)  # ab通道和l通道拼接
    img_rgb = color.lab2rgb(img_lab)

    return img_rgb

if __name__=="__main__":
    x = paddle.randn([40, 313, 56, 56])
